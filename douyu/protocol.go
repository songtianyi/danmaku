...
const (
	...
	MSG_TYPE_LUCK_GUY = "onlinegift" //领取在线鱼丸暴击消息
	...
	MSG_TYPE_USER_ENTER = "uenter" // 特殊用户进房通知消息
	...
	//     dc     酬勤数量:默认值 0(表示没有酬勤数量)
	//     bdlv   最高酬勤等级:默认值 0(表示全站都没有酬勤)
	MSG_TYPE_NEW_DESERVE = "bc_buy_deserve" // 用户赠送酬勤通知消息
	...
	MSG_TYPE_LIVE_STATUS_CHANGE = "rss" // 房间开关播提醒消息
	// 房间开播提醒主要部分应包含的字段如下:
	// 字段说明
	...
	//     notify  通知类型
	//     endtime 关播时间(仅关播时有效)
	MSG_TYPE_RANKLIST = "ranklist" // 广播排行榜消息
	MSG_TYPE_SSD      = "ssd"      // 超级弹幕消息(如，火箭弹幕)
	// 超级弹幕主要部分应包含的字段如下:
	// 字段说明
	...
	//     gfid   礼物 id
	//     eid    特效 id
	MSG_TYPE_RED_PACKET = "ggbb" // 房间用户抢红包
	...
	//     dnk   抢礼包者昵称
	//     rpt   礼包类型
	MSG_TYPE_RANK_UP = "rankup" // 房间内top10变化消息
